...
const sameText = (a, b) => upper(a).trim() === upper(b).trim();
const textValue = (value) => upper(value || '');
const textInputTypes = new Set(['text', undefined]);
...
export function WBSEditPage() {
  ...
  const set = (k, v) => setForm(prev => ({ ...prev, [k]: typeof v === 'string' ? upper(v) : v }));
  ...
  const loadOrder = async (order) => {
    ...
    const selectedOrder = upper(order).trim();
    set('supply_order_no', selectedOrder);
    const params = new URLSearchParams({ project_name: form.project_name, supply_order_no: selectedOrder });
    const res = await fetch(`${apiBase()}/wbs/get?${params}`, { headers: authHeaders() });
    const data = await res.json();
    setForm(Object.fromEntries(Object.entries({ ...blank, ...(data.record || {}), project_name: form.project_name, supply_order_no: selectedOrder }).map(([k, v]) => [k, typeof v === 'string' ? upper(v) : v])));
  };
  ...
  const submit = async (e) => {
    ...
    setStatus('');
    const payload = {
      ...Object.fromEntries(Object.entries(form).map(([k, v]) => [k, typeof v === 'string' ? upper(v) : v])),
        ...
        <div className={styles.card}>
          <form className={styles.form} onSubmit={submit}>
            <div className={styles.formGrid2}>
              <label className={styles.label}>PROJECT NAME<select className={styles.control} value={form.project_name} onChange={e => { setForm({ ...blank, project_name: e.target.value }); setMode(''); }} required><option value="">SELECT PROJECT</option>{projects.map(p => <option key={p} value={p}>{upper(p)}</option>)}</select></label>
              {form.project_name && <label className={styles.label}>SUPPLY ORDER NO<select className={styles.control} value={mode === 'new' ? '__new__' : form.supply_order_no} onChange={e => loadOrder(e.target.value)} required><option value="">SELECT SUPPLY ORDER</option>{orders.map(o => <option key={o} value={upper(o)}>{upper(o)}</option>)}<option value="__new__">CREATE NEW SUPPLY ORDER NO.</option></select></label>}
            ...
            {(mode || form.supply_order_no) && <div className={styles.formGrid2}>
              ...
              ].map(([k,l,t]) => <label key={k} className={styles.label}>{l}<input className={styles.control} type={t || 'text'} min={t === 'number' ? '1' : undefined} value={textInputTypes.has(t) ? textValue(form[k]) : (form[k] || '')} onChange={e => set(k, e.target.value)} /></label>)}
              ...
              <label className={styles.label}>TYPE<select className={styles.control} value={form.type} onChange={e => set('type', e.target.value)}><option value="">SELECT TYPE</option><option value="WARRANTY">WARRANTY</option><option value="CAMC">CAMC</option><option value="FRRC">FRRC</option></select></label>
              <label className={styles.label}>STATUS OF WBS<select className={styles.control} value={form.status_of_wbs || 'OPEN'} onChange={e => set('status_of_wbs', e.target.value)}><option value="OPEN">OPEN</option><option value="CLOSE">CLOSE</option></select></label>
            </div>}
...
export function WBSViewPage() {
  ...
  const filtered = records.filter(r => (!project || sameText(r.project_name, project)) && (!order || r.supply_order_no === order));
  const load = async () => {
    setStatus('');
  ...
  return <Shell><div className={styles.page}><SectionNav section="wbs" /><div className={styles.pageHeader}><div className={styles.pageTitle}>WBS DETAILS - VIEW</div><button className={`${styles.btn} ${styles.btnGhost}`} onClick={() => navigate('/wbs')}>BACK</button></div><div className={styles.card}><div className={styles.formGrid2}><label className={styles.label}>PROJECT NAME<select className={styles.control} value={project} onChange={e => { setProject(e.target.value); setOrder(''); }}><option value="">ALL PROJECTS</option>{projects.map(p => <option key={p} value={p}>{upper(p)}</option>)}</select></label><label className={styles.label}>SUPPLY ORDER NO<select className={styles.control} value={order} onChange={e => setOrder(e.target.value)}><option value="">ALL SUPPLY ORDERS</option>{orders.map(o => <option key={o} value={o}>{upper(o)}</option>)}</select></label></div><div className={styles.pageActions} style={{ marginTop: 12 }}><button className={`${styles.btn} ${styles.btnPrimary}`} type="button" onClick={load}>SEARCH</button><button className={`${styles.btn} ${styles.btnGhost}`} type="button" onClick={reset}>RESET</button>{searched && filtered.length > 0 && <button className={`${styles.btn} ${styles.btnPrimary}`} type="button" onClick={download}>DOWNLOAD</button>}</div>{status && <div style={{ marginTop: 12 }}>{upper(status)}</div>}</div>{searched && <div className={styles.card} style={{ marginTop: 16 }}><div className={styles.pageTitle} style={{ marginBottom: 12 }}>RESULTS ({filtered.length})</div><div className={styles.tableWrap}><table className={styles.table}><thead><tr>{['Project Name','Supply Order No.','Date of SO','Customer Details','Contract Number','Sale Order','Project Details','WBS Number','Type','Duration','Date of Start','Date of End','Status of WBS','Amount Sanctioned (INR)','Remaining Amount (INR)','Remarks','Created By','Updated By'].map(h => <th key={h}>{h}</th>)}</tr></thead><tbody>{filtered.length ? filtered.map(r => <tr key={r._id}><td>{upper(r.project_name)}</td><td>{upper(r.supply_order_no)}</td><td>{r.date_of_so}</td><td>{upper(r.customer_details)}</td><td>{r.contract_number}</td><td>{r.sale_order}</td><td>{upper(r.project_details)}</td><td>{upper(r.wbs_number)}</td><td>{upper(r.type)}</td><td>{r.duration}</td><td>{r.date_of_start}</td><td>{r.date_of_end}</td><td>{upper(r.status_of_wbs)}</td><td>{formatInr(r.amount_sanctioned_inr)}</td><td>{formatInr(r.remaining_amount_inr)}</td><td>{upper(r.remarks)}</td><td>{upper(r.created_by)}</td><td>{upper(r.updated_by)}</td></tr>) : <tr><td colSpan="11">NO WBS RECORDS FOUND.</td></tr>}</tbody></table></div></div>}</div></Shell>;
}
